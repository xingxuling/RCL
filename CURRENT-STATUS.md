...
- Whole-language runtime self-hosting is not claimed. Six Foundation domains now execute through RBC 1.2 and the `RclVmProviderV1` Native Provider bridge; uncovered and declared-domain runtime remains JavaScript.
...
- Verification entrypoint: `npm run verify:version-contract`; the bridge gates are `npm run test:foundation-native-batch-a` and `npm run conformance:foundation`.
