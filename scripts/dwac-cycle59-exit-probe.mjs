...
steps.push(
  ...
  { code: 54, name: 'deployment-health', args: ['scripts/verify-vercel-health-evidence.mjs'] },
);
...
for (const step of steps) {
  ...
  if (result.error || result.status !== 0) {
    ...
    console.error(JSON.stringify({
      status: 'DWAC_CYCLE59_EXIT_PROBE_FAILURE',
...
console.log(JSON.stringify({ ok: true, status: 'DWAC_CYCLE59_EXIT_PROBE_ALL_PASS' }, null, 2));
