...
function fail(message, details = {}) {
  console.error(JSON.stringify({
    ok: false,
    status: 'RCL_FOUNDATION_CAPABILITY_TRUTH_SURFACE_VERIFICATION_FAILED',
    message,
    ...details,
  }, null, 2));
  process.exit(1);
...
if (surface.ok !== true) {
  fail('Canonical runtime capability truth surface did not verify.', { surface });
}
if (!/^[0-9a-f]{64}$/i.test(surface.truthRoot ?? '')) {
  fail('Runtime capability truth surface does not expose a content-addressed truth root.', { surface });
}
...
if (surface.providerBridge.domainCount !== 16 || surface.providerBridge.batchCount !== 5) {
  fail('Runtime Provider bridge topology drifted.', { providerBridge: surface.providerBridge });
}
if (
  ...
  fail('Runtime capability truth surface overclaimed execution guarantees.', {
...
for (const mutation of [
  {
    ...
    apply(value) {
      value.truthRoot = '0'.repeat(64);
    },
  },
  {
    id: 'direct-registry-root-mutation',
    apply(value) {
      value.direct.registryRoot = '1'.repeat(64);
    },
  },
  {
    id: 'provider-bridge-domain-mutation',
    apply(value) {
      value.providerBridge.domains = [...value.providerBridge.domains, 'phantom-domain'];
    },
  },
  {
    id: 'all-domain-native-overclaim',
    apply(value) {
      value.truthBoundary.allFoundationDomainsDirectNativeClaimed = true;
    },
  },
]) {
  ...
  negativeControls.push({
    id: mutation.id,
    detected,
    errorCodes: mutatedSurface.errors.map(error => error.code),
  });
  if (!detected) {
    fail(`Negative control ${mutation.id} was not detected.`, { mutatedSurface });
  }
}
