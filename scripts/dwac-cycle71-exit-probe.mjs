#!/usr/bin/env node
import { spawnSync } from 'node:child_process';

const steps = [
  { code: 93, name: 'cycle70-regression-proof-chain', args: ['scripts/dwac-cycle70-exit-probe.mjs'] },
  { code: 94, name: 'runtime-health-statepath-negative-controls', args: ['scripts/verify-runtime-health-statepath-negative-control.mjs'] },
  { code: 95, name: 'runtime-health-statepath-truth-binding', args: ['scripts/verify-vercel-runtime-health-truth.mjs'] },
];

for (const step of steps) {
  const result = spawnSync(process.execPath, step.args, { stdio: 'inherit', env: process.env });
  if (result.error || result.status !== 0) {
    console.error(JSON.stringify({
      status: 'DWAC_CYCLE71_EXIT_PROBE_FAILURE',
      probeExitCode: step.code,
      step: step.name,
      childExitCode: result.status ?? null,
      error: result.error?.message ?? null,
    }, null, 2));
    process.exit(step.code);
  }
}

console.log(JSON.stringify({
  ok: true,
  status: 'DWAC_CYCLE71_EXIT_PROBE_ALL_PASS',
  truthBoundary: {
    runtimeHealthFailsClosedOnProviderBridgeStatePathSemanticDrift: true,
    runtimeHealthBindsCanonicalProviderBridgeStatePathAttestationRoot: true,
    providerBridgeTopologyAloneDoesNotClaimStatePathSemanticParity: true,
    statePathSemanticAttestationDoesNotImplyDirectNativeExecution: true,
  },
}, null, 2));
