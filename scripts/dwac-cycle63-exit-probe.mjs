...
{code:36,name:'runtime-capability-truth',args:['scripts/verify-foundation-capability-truth-surface.mjs']},
{code:37,name:'canonical-native-artifact',args:['scripts/build-vercel-native.mjs']},
{code:38,name:'quantitative-direct-real-c',args:['scripts/verify-vercel-foundation-quantitative-direct.mjs']},
{code:39,name:'batch-a-real-c-provider',args:['scripts/verify-vercel-foundation-batch-a-bridge.mjs']},
{code:40,name:'batch-a-deployment-binding',args:['scripts/bind-vercel-foundation-batch-a-bridge.mjs']},
{code:41,name:'base-federation-conformance',args:['scripts/foundation-conformance-base.mjs','--out','public/foundation-native-federation-conformance-base']},
{code:null,propagateChildStatus:true,name:'conformance-truth-field-diff',args:['scripts/dwac-cycle58-truth-diff-probe.mjs']},
{code:42,name:'full-federation-conformance',args:['scripts/foundation-conformance.mjs','--out','public/foundation-native-federation-conformance']},
{code:43,name:'canonical-conformance-truth',args:['scripts/verify-foundation-conformance-truth.mjs','--out','public/foundation-native-federation-conformance']},
{code:44,name:'developer-release-conformance-truth',args:['scripts/verify-developer-release-conformance-truth.mjs']},
{code:45,name:'strict-federation-extension',args:['scripts/verify-vercel-foundation-native-federation-extension.mjs']},
{code:46,name:'physical-real-c',args:['scripts/verify-vercel-foundation-physical.mjs']},
{code:47,name:'neural-real-c',args:['scripts/verify-vercel-foundation-neural.mjs']},
{code:48,name:'neural-deployment-binding',args:['scripts/bind-vercel-foundation-neural.mjs']},
{code:49,name:'genetic-real-c',args:['scripts/verify-vercel-foundation-genetic.mjs']},
...
for (const step of steps) { const result=spawnSync(process.execPath,step.args,{stdio:'inherit',env:process.env}); if(result.error||result.status!==0){const exitCode=step.propagateChildStatus&&Number.isInteger(result.status)?result.status:step.code; console.error(JSON.stringify({status:'DWAC_CYCLE63_EXIT_PROBE_FAILURE',probeExitCode:exitCode,step:step.name,childExitCode:result.status??null,error:result.error?.message??null},null,2)); process.exit(exitCode??1);} }
console.log(JSON.stringify({ok:true,status:'DWAC_CYCLE63_EXIT_PROBE_ALL_PASS'},null,2));
