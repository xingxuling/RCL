...
const k326RuntimeContractPath = 'examples/universal-stress/k326-compiler-database-runtime-contract.v0.1.json';
const k326RuntimePath = 'examples/universal-stress/evidence/k326-compiler-database-runtime-v0.1.json';
const k326AiContractPath = 'examples/universal-stress/k326-compiler-database-ai-generation-contract.v0.1.json';
const k326AiReceiptPath = 'examples/universal-stress/evidence/k326-compiler-database-ai-generate/receipt.json';
const k326AiGithubReplayPath = 'examples/universal-stress/evidence/k326-compiler-database-ai-generate/github-replay.json';
const k336RuntimeContractPath = 'examples/universal-stress/k336-compiler-automation-runtime-contract.v0.1.json';
const k336RuntimePath = 'examples/universal-stress/evidence/k336-compiler-automation-runtime-v0.1.json';
const k336AiContractPath = 'examples/universal-stress/k336-compiler-automation-ai-generation-contract.v0.1.json';
const k336AiReceiptPath = 'examples/universal-stress/evidence/k336-compiler-automation-ai-generate/receipt.json';
const k336AiGithubReplayPath = 'examples/universal-stress/evidence/k336-compiler-automation-ai-generate/github-replay.json';
const k334RuntimeContractPath = 'examples/universal-stress/k334-compiler-agent-runtime-contract.v0.1.json';
const k334RuntimePath = 'examples/universal-stress/evidence/k334-compiler-agent-runtime-v0.1.json';
...
const k326AiAdmitted = k326Ai.aiGenerateAdmission === 'PASS';
const k336Ai = verifyK336CompilerAutomationReceipt();
...
if (k326AiAdmitted) {
  const sharedEvidence = [k326RuntimeContractPath, k326RuntimePath, k326AiContractPath, k326AiReceiptPath, k326AiGithubReplayPath];
  const gates = Object.fromEntries(UNIVERSAL_STRESS_GATES.map((gate) => [gate, {
    status: STRESS_STATUS.PASS,
    evidence: sharedEvidence,
    note: gate === 'AI_GENERATE'
      ? `Three independent primary-key/foreign-key/domain-constraint repairs passed; GitHub run ${k326Ai.githubAuthority.runId} bound focused and Windows replay.`
      : `Frozen 20-round native relational-transaction receipt ${k326Ai.runtimeEvidenceBinding.reportRoot}.`,
  }]));
  claimsById.set('compiler-runtime::database', {
    id: 'compiler-runtime::database',
    coverageMode: COVERAGE_MODE.NATIVE_SEMANTIC,
    lastVerifiedSha: k326Ai.githubAuthority.sourceCommit,
    lastVerifiedDate: k326Ai.githubAuthority.verifiedAt.slice(0, 10),
    knownLimits: [
      'The database claim is limited to one bounded in-memory integer relation profile with primary/foreign keys, selection, join, aggregation and atomic insert rollback.',
      'SQL parsing, durable storage, concurrency, isolation levels, query planning, external database services and unrelated K400 cells remain unverified.',
    ],
    relatedKillerTasks: ['K05'],
    requiredGenes: ['relation-sequences', 'primary-foreign-key-constraints', 'selection-join-aggregation', 'atomic-commit-rollback', 'native-compiler-runtime'],
    gates: structuredClone(gates),
    changes: [{
      id: 'compiler-database-relational-transaction-profile',
      kind: 'stress-evidence',
      scope: ['compiler-runtime'],
      generalPrimitive: true,
      justification: 'RCL owns relation rows, constraints, queries and atomic transaction semantics while native rclc/rclvm execute without an opaque database provider.',
    }],
  });
}
if (k336AiAdmitted) {
  const sharedEvidence = [k336RuntimeContractPath, k336RuntimePath, k336AiContractPath, k336AiReceiptPath, k336AiGithubReplayPath];
  const gates = Object.fromEntries(UNIVERSAL_STRESS_GATES.map((gate) => [gate, {
    status: STRESS_STATUS.PASS,
    evidence: sharedEvidence,
    note: gate === 'AI_GENERATE'
      ? `Three independent dependency/retry/approval repairs passed; GitHub run ${k336Ai.githubAuthority.runId} bound focused and Windows replay.`
      : `Frozen 20-round native governed-automation receipt ${k336Ai.runtimeEvidenceBinding.reportRoot}.`,
  }]));
  claimsById.set('compiler-runtime::automation', {
    id: 'compiler-runtime::automation',
    coverageMode: COVERAGE_MODE.NATIVE_SEMANTIC,
    lastVerifiedSha: k336Ai.githubAuthority.sourceCommit,
    lastVerifiedDate: k336Ai.githubAuthority.verifiedAt.slice(0, 10),
    knownLimits: [
      'The automation claim is limited to one bounded deterministic dependency/retry/approval/kill-switch/compensation workflow profile.',
      'External actions, wall-clock scheduling, arbitrary DAGs, durable queues and production workflow authority remain unverified.',
    ],
    relatedKillerTasks: ['K09'],
    requiredGenes: ['dependency-gate', 'bounded-retry', 'human-approval', 'kill-switch', 'compensating-rollback', 'native-compiler-runtime'],
    gates: structuredClone(gates),
    changes: [{
      id: 'compiler-automation-governed-workflow-profile',
      kind: 'stress-evidence',
      scope: ['compiler-runtime'],
      generalPrimitive: true,
      justification: 'RCL owns dependency, retry, approval, kill-switch, audit, commit and compensation semantics while native rclc/rclvm execute without an external action provider.',
    }],
  });
}
if (k334AiAdmitted) {
  const sharedEvidence = [k334RuntimeContractPath, k334RuntimePath, k334AiContractPath, k334AiReceiptPath, k334AiGithubReplayPath];
  const gates = Object.fromEntries(UNIVERSAL_STRESS_GATES.map((gate) => [gate, {
    status: STRESS_STATUS.PASS,
    evidence: sharedEvidence,
    note: gate === 'AI_GENERATE'
      ? `Three independent capability/budget/approval repairs passed; GitHub run ${k334Ai.githubAuthority.runId} bound focused and Windows replay.`
      : `Frozen 20-round native governed-agent receipt ${k334Ai.runtimeEvidenceBinding.reportRoot}.`,
  }]));
  claimsById.set('compiler-runtime::agent', {
    id: 'compiler-runtime::agent',
    coverageMode: COVERAGE_MODE.NATIVE_SEMANTIC,
    lastVerifiedSha: k334Ai.githubAuthority.sourceCommit,
    lastVerifiedDate: k334Ai.githubAuthority.verifiedAt.slice(0, 10),
    knownLimits: [
      'The agent claim is limited to one bounded deterministic observation/action selection, capability, budget, risk, approval, kill-switch, memory and commit profile.',
      'LLM reasoning, external tool execution, network access, durable memory and autonomous production authority remain unverified.',
    ],
    relatedKillerTasks: ['K08'],
    requiredGenes: ['action-deliberation', 'capability-gate', 'budget-and-risk-bounds', 'human-approval', 'kill-switch', 'bounded-memory', 'native-compiler-runtime'],
    gates: structuredClone(gates),
    changes: [{
      id: 'compiler-agent-governed-deliberation-profile',
      kind: 'stress-evidence',
      scope: ['compiler-runtime'],
      generalPrimitive: true,
      justification: 'RCL owns deterministic action scoring, capability, budget, risk, approval, memory and commit semantics while native rclc/rclvm execute without an LLM or external tool provider.',
    }],
  });
}
if (k331AiAdmitted) {
...
const evidence = {
  ...
  sourceReceipts: [
    ...
    k326RuntimeContractPath,
    k326RuntimePath,
    k326AiContractPath,
    k326AiReceiptPath,
    ...(k326AiAdmitted ? [k326AiGithubReplayPath] : []),
    k336RuntimeContractPath,
    k336RuntimePath,
    k336AiContractPath,
    k336AiReceiptPath,
    ...(k336AiAdmitted ? [k336AiGithubReplayPath] : []),
    k334RuntimeContractPath,
    k334RuntimePath,
    k334AiContractPath,
    k334AiReceiptPath,
  ...
  notes: [
    ...
    k326AiAdmitted
      ? `K326 closes compiler-runtime::database through 20/20 native relational-transaction rounds, 3/3 independent repairs and GitHub run ${k326Ai.githubAuthority.runId}.`
      : 'K326 has 20/20 rooted native relational-transaction rounds and a 3/3 independently replayed local repair candidate; it remains UNTESTED until GitHub focused and Windows replay are bound.',
    k336AiAdmitted
      ? `K336 closes compiler-runtime::automation through 20/20 native governed-workflow rounds, 3/3 independent repairs and GitHub run ${k336Ai.githubAuthority.runId}.`
      : 'K336 has 20/20 rooted native governed-workflow rounds and a 3/3 independently replayed local repair candidate; it remains UNTESTED until GitHub focused and Windows replay are bound.',
    k334AiAdmitted
