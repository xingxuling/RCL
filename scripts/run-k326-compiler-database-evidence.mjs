...
const DEFAULT_CONTRACT_PATH = path.join(ROOT, 'examples', 'universal-stress', 'k326-compiler-database-runtime-contract.v0.1.json');
const DEFAULT_OUTPUT_PATH = path.join(ROOT, 'examples', 'universal-stress', 'evidence', 'k326-compiler-database-runtime-v0.1.json');
...
function project(state) {
  return {
    customerCount: state?.['query.region_10_customer_count'],
    initialSum: state?.['query.region_10_initial_sum'],
    initialJoinCount: state?.['query.region_10_initial_join_count'],
    duplicateCommitted: state?.['transaction.duplicate']?.[1],
    duplicateLength: state?.['transaction.duplicate']?.[0]?.length,
    orphanCommitted: state?.['transaction.orphan']?.[1],
    orphanLength: state?.['transaction.orphan']?.[0]?.length,
    negativeCommitted: state?.['transaction.negative']?.[1],
    negativeLength: state?.['transaction.negative']?.[0]?.length,
    validCommitted: state?.['transaction.valid']?.[1],
    validLength: state?.['transaction.valid']?.[0]?.length,
    committedSum: state?.['query.region_10_committed_sum'],
    committedJoinCount: state?.['query.region_10_committed_join_count'],
    evaluationPass: state?.['evaluation.pass'],
  };
}
function independentOracle() {
  ...
  const insert = (row) => orders.some((item) => item.id === row.id)
    || !customers.some((item) => item.id === row.customer)
    || row.amount < 0
    ? { rows: orders, committed: 0 }
    : { rows: [...orders, row], committed: 1 };
  const aggregate = (rows) => rows.filter((order) => order.approved === 1
    && customers.some((customer) => customer.id === order.customer && customer.region === 10));
  const duplicate = insert({ id: 103, customer: 2, amount: 55, approved: 1 });
  const orphan = insert({ id: 104, customer: 99, amount: 25, approved: 1 });
  const negative = insert({ id: 104, customer: 3, amount: -25, approved: 1 });
  const valid = insert({ id: 104, customer: 3, amount: 25, approved: 1 });
  return {
    customerCount: customers.filter((row) => row.region === 10).length,
    initialSum: aggregate(orders).reduce((sum, row) => sum + row.amount, 0),
    initialJoinCount: aggregate(orders).length,
    duplicateCommitted: duplicate.committed,
    duplicateLength: duplicate.rows.length,
    orphanCommitted: orphan.committed,
    orphanLength: orphan.rows.length,
    negativeCommitted: negative.committed,
    negativeLength: negative.rows.length,
    validCommitted: valid.committed,
    validLength: valid.rows.length,
    committedSum: aggregate(valid.rows).reduce((sum, row) => sum + row.amount, 0),
    committedJoinCount: aggregate(valid.rows).length,
    evaluationPass: true,
  };
}

export function runK326CompilerDatabaseEvidence(options = {}) {
  ...
  if (contract.format !== 'rcl.k326.compiler-database-runtime-contract.v0.1'
    || contract.frozenBeforeAcquisition !== true) throw new Error('RCL_K326_RUNTIME_CONTRACT_INVALID');
  ...
  for (const requiredPath of [sourcePath, compilerRbcPath, rclcPath, rclvmPath]) {
    if (!fs.existsSync(requiredPath)) throw new Error(`RCL_K326_REQUIRED_ARTIFACT_MISSING:${requiredPath}`);
  }
  const source = fs.readFileSync(sourcePath, 'utf8');
  if (sha256(source) !== contract.canonical.sourceSha256) throw new Error('RCL_K326_CANONICAL_SOURCE_DRIFT');
  if (sha256(fs.readFileSync(compilerRbcPath)) !== contract.canonical.compilerRbcSha256) throw new Error('RCL_K326_COMPILER_RBC_DRIFT');
  if (JSON.stringify(contract.expectedProjection) !== JSON.stringify(independentOracle())) throw new Error('RCL_K326_CONTRACT_ORACLE_DRIFT');

  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rcl-k326-runtime-'));
  ...
  try {
    ...
    if (sha256(bootstrapBytecode) !== contract.canonical.bootstrapArtifactSha256) throw new Error('RCL_K326_BOOTSTRAP_ARTIFACT_DRIFT');
    for (let index = 0; index < contract.required.rounds; index += 1) {
      ...
      if (compilation.error || compilation.status !== 0) throw new Error(`RCL_K326_NATIVE_COMPILE_FAILED:${index}:${compilation.stderr}`);
      const nativeBytecode = fs.readFileSync(rbcPath);
      if (contract.required.bootstrapByteParity && !nativeBytecode.equals(bootstrapBytecode)) throw new Error(`RCL_K326_BOOTSTRAP_PARITY_FAILED:${index}`);
      const execution = run(rclvmPath, [rbcPath]);
      if (execution.error || execution.status !== 0) throw new Error(`RCL_K326_NATIVE_EXECUTE_FAILED:${index}:${execution.stderr}`);
      const payload = verifyNativeSemanticStateRoot(parseJson(execution.stdout, 'RCL_K326_NATIVE_JSON_INVALID'), {
        requireNativeRoot: contract.required.nativeSemanticStateRoot,
      });
      const observed = project(payload.state);
      if (JSON.stringify(observed) !== JSON.stringify(contract.expectedProjection)) throw new Error(`RCL_K326_EXPECTED_STATE_MISMATCH:${index}`);
      if (contract.required.independentJavascriptOracle && JSON.stringify(observed) !== JSON.stringify(independentOracle())) throw new Error(`RCL_K326_ORACLE_MISMATCH:${index}`);
    ...
    const controls = [
      ['primaryKeyMutationRejected', 'order_id(sequence_get(orders, cursor)) == id', 'order_id(sequence_get(orders, cursor)) != id'],
      ['foreignKeyMutationRejected', 'not has_customer(customers, 0, order_customer(row))', 'has_customer(customers, 0, order_customer(row))'],
      ['aggregateMutationRejected', '        order_amount(sequence_get(orders, cursor)),\n        0) + sum_approved_orders_for_region', '        0 - order_amount(sequence_get(orders, cursor)),\n        0) + sum_approved_orders_for_region'],
      ['rollbackMutationRejected', 'make_transaction(orders, 0)', 'make_transaction(sequence_append(orders, row), 0)'],
    ];
    for (const [name, oldText, newText] of controls) {
      const mutatedSource = replaceExactlyOnce(source, oldText, newText, `RCL_K326_MUTATION_SITE_INVALID:${name}`);
      ...
      const payload = verifyNativeSemanticStateRoot(parseJson(execution.stdout, `RCL_K326_${name}_JSON_INVALID`), { requireNativeRoot: true });
  ...
  const withoutRoot = {
    format: 'rcl.k326.compiler-database-runtime-evidence.v0.1',
    ...
    oracle: { implementation: 'independent-javascript-relational-filter-join-aggregate-and-insert', expected: independentOracle() },
    rounds,
    negativeControls,
    summary: {
...
if (isMain) {
  const result = runK326CompilerDatabaseEvidence();
