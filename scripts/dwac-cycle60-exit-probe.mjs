...
steps.push(
  ...
  { code: 52, name: 'quantitative-direct-deployment-binding', args: ['scripts/bind-vercel-foundation-quantitative-direct.mjs'] },
  { code: 53, name: 'quantitative-direct-health', args: ['scripts/verify-vercel-foundation-quantitative-direct-health.mjs'] },
  { code: 54, name: 'energy-runtime-capability-truth', args: ['scripts/verify-foundation-energy-deployment-truth.mjs'] },
  { code: 55, name: 'deployment-health', args: ['scripts/verify-vercel-health-evidence.mjs'] },
);
...
for (const step of steps) {
  ...
  if (result.error || result.status !== 0) {
    ...
    console.error(JSON.stringify({
      status: 'DWAC_CYCLE60_EXIT_PROBE_FAILURE',
...
console.log(JSON.stringify({ ok: true, status: 'DWAC_CYCLE60_EXIT_PROBE_ALL_PASS' }, null, 2));
