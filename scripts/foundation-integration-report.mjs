...
const verification = [
  ...
  testResult('RCL Conformance', 'npm run conformance:foundation', 'pass', '61/61 checks; six Batch A domains, three Meta Batch B planes, and two Batch C domains verified through Native Provider ABI bridges'),
  ...
  testResult('RCL Foundation Native Batch C', 'npm run test:foundation-native-batch-c', 'pass', '4/4 tests; physical-to-embodiment causal chain, RBC 1.2 self-host parity, counterfactual, negative gates and performance baseline'),
...
const blocked = [
  ...
  { item: 'Remaining declared Foundation syntax in RCL Native VM', reason: 'Batch A, Meta Batch B and Batch C are verified through RclVmProviderV1 in bridge mode; declared-domain lowering and uncovered modules remain explicitly unsupported.' },
];

const knownLimitations = [
  'RCL Native Providers cover quantitative, knowledge, perception, natural-language, understanding, creative, meta-spacetime, meta-acceleration, meta-compression, physical and embodiment in bridge mode; declared-domain syntax and the remaining modules are not native yet.',
...
function markdown(data) {
  ...
  return [
    ...
    '- RCL Batch A, Meta Batch B and Batch C execute eleven Foundation entries through three verified RclVmProviderV1 bridges.',
