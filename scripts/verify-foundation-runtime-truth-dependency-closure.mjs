...
function fail(message, details = {}) {
  console.error(JSON.stringify({ ok: false, status: 'RCL_RUNTIME_TRUTH_DEPENDENCY_CLOSURE_FAILED', message, ...details }, null, 2));
  process.exit(1);
}
function clone(value) { return JSON.parse(JSON.stringify(value)); }
function expectRejected(label, candidate) {
...
try {
  ...
  if (verification.ok !== true) fail('Canonical runtime truth contract dependency closure did not verify.', { verification });
  ...
  for (const requiredPath of requiredTransitive) {
    if (!paths.includes(requiredPath) || !bindings.has(requiredPath)) {
      fail('A canonical runtime-truth transitive dependency is not dependency-closed/content-bound.', { requiredPath, paths });
    }
  }
  if (contract.dependencyClosure?.unresolvedRelativeImports?.length !== 0) {
    fail('Canonical runtime-truth dependency closure contains unresolved repository-local imports.', { dependencyClosure: contract.dependencyClosure });
  }
  ...
  const missingDependencyResult = expectRejected('transitive-dependency-binding-removed', missingDependency);
  ...
  const sourceHashDriftResult = expectRejected('transitive-dependency-source-hash-drift', sourceHashDrift);
  ...
  const boundaryResult = expectRejected('dynamic-resource-boundary-removed', boundaryOverclaim);
  ...
  fail(error?.message ?? String(error), { code: error?.code ?? null, details: error?.details ?? null, stack: error?.stack ?? null });
}
