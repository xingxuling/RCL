...
const DEFAULT_CONTRACT_PATH = path.join(ROOT, 'examples', 'universal-stress', 'k333-compiler-machine-learning-runtime-contract.v0.1.json');
const DEFAULT_OUTPUT_PATH = path.join(ROOT, 'examples', 'universal-stress', 'evidence', 'k333-compiler-machine-learning-runtime-v0.1.json');
...
function oracleState() {
  const dataset = [[1, 1, 0], [3, 1, 1], [1, 3, 1], [3, 3, 1]];
  let parameters = [0, 0, 0];
  const classify = (nodes, hotness) => (parameters[0] * nodes + parameters[1] * hotness + parameters[2] >= 0 ? 1 : 0);
  for (let epoch = 0; epoch < 16; epoch += 1) {
    for (const [nodes, hotness, label] of dataset) {
      if (classify(nodes, hotness) === label) continue;
      const direction = label === 1 ? 1 : -1;
      parameters = [parameters[0] + direction * nodes, parameters[1] + direction * hotness, parameters[2] + direction];
    }
  }
  const correct = dataset.filter(([nodes, hotness, label]) => classify(nodes, hotness) === label).length;
  const score = parameters[0] * 4 + parameters[1] * 2 + parameters[2];
  return {
    'training.final_parameters': parameters,
    'evaluation.accuracy': correct / dataset.length,
    'inference.score': score,
    'inference.classification': score >= 0 ? 1 : 0,
    'inference.recommendation': score >= 0 ? 'EXPENSIVE_OPTIMIZATION_CANDIDATE' : 'STANDARD_OPTIMIZATION_CANDIDATE',
    'boundary.small_cold_classification': classify(1, 1),
    'authority.model_commit_granted': false,
    'evaluation.pass': true,
  };
}

export function runK333CompilerMachineLearningEvidence(options = {}) {
  ...
  if (contract.format !== 'rcl.k333.compiler-machine-learning-runtime-contract.v0.1'
    || contract.frozenBeforeAcquisition !== true) throw new Error('RCL_K333_RUNTIME_CONTRACT_INVALID');
  ...
  for (const requiredPath of [sourcePath, compilerRbcPath, rclcPath, rclvmPath]) {
    if (!fs.existsSync(requiredPath)) throw new Error(`RCL_K333_REQUIRED_ARTIFACT_MISSING:${requiredPath}`);
  }
  const source = fs.readFileSync(sourcePath, 'utf8');
  if (sha256(source) !== contract.canonical.sourceSha256) throw new Error('RCL_K333_CANONICAL_SOURCE_DRIFT');
  if (sha256(fs.readFileSync(compilerRbcPath)) !== contract.canonical.compilerRbcSha256) throw new Error('RCL_K333_COMPILER_RBC_DRIFT');
  if (!expectedMatches(contract.expectedState, oracleState())) throw new Error('RCL_K333_CONTRACT_ORACLE_DRIFT');

  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rcl-k333-runtime-'));
  ...
  try {
    ...
    if (sha256(bootstrapBytecode) !== contract.canonical.bootstrapArtifactSha256) throw new Error('RCL_K333_BOOTSTRAP_ARTIFACT_DRIFT');
    for (let index = 0; index < contract.required.rounds; index += 1) {
      ...
      if (compilation.error || compilation.status !== 0) throw new Error(`RCL_K333_NATIVE_COMPILE_FAILED:${index}:${compilation.stderr}`);
      const nativeBytecode = fs.readFileSync(rbcPath);
      if (contract.required.bootstrapByteParity && !nativeBytecode.equals(bootstrapBytecode)) throw new Error(`RCL_K333_BOOTSTRAP_PARITY_FAILED:${index}`);
      const execution = run(rclvmPath, [rbcPath]);
      if (execution.error || execution.status !== 0) throw new Error(`RCL_K333_NATIVE_EXECUTE_FAILED:${index}:${execution.stderr}`);
      const payload = verifyNativeSemanticStateRoot(parseJson(execution.stdout, 'RCL_K333_NATIVE_JSON_INVALID'), {
        requireNativeRoot: contract.required.nativeSemanticStateRoot,
      });
      if (!expectedMatches(payload.state, contract.expectedState)) throw new Error(`RCL_K333_EXPECTED_STATE_MISMATCH:${index}`);
      if (contract.required.independentJavascriptOracle && !expectedMatches(payload.state, oracleState())) throw new Error(`RCL_K333_ORACLE_MISMATCH:${index}`);
    ...
    const controls = [
      ['trainingLabelMutationRejected', 'row(1, 1, 0)', 'row(1, 1, 1)'],
      ['perceptronUpdateSignMutationRejected', 'node_weight(parameters) + choose', 'node_weight(parameters) - choose'],
      ['zeroEpochNoTraining', 'facet training.epochs : Number = 16', 'facet training.epochs : Number = 0'],
      ['modelCommitAuthorityRemainsFalse', 'facet authority.model_commit_granted : Truth = false', 'facet authority.model_commit_granted : Truth = true'],
    ];
    for (const [name, oldText, newText] of controls) {
      const mutatedSource = replaceExactlyOnce(source, oldText, newText, `RCL_K333_MUTATION_SITE_INVALID:${name}`);
      ...
      const payload = verifyNativeSemanticStateRoot(parseJson(execution.stdout, `RCL_K333_${name}_JSON_INVALID`), { requireNativeRoot: true });
  ...
  const payloadWithoutRoot = {
    format: 'rcl.k333.compiler-machine-learning-runtime-evidence.v0.1',
    ...
    oracle: { implementation: 'independent-integer-perceptron-javascript', expected: oracleState() },
    rounds,
    negativeControls,
    summary: {
...
if (isMain) {
  const result = runK333CompilerMachineLearningEvidence();
