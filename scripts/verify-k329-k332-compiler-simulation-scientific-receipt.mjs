...
import { K329_K332_AI_GENERATION_MUTATIONS } from './run-k329-k332-independent-ai-generation.mjs';
import { verifyK329K332CompilerSimulationScientificCandidate } from './verify-k329-k332-compiler-simulation-scientific-candidate.mjs';

const ROOT = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const DEFAULT_CONTRACT_PATH = path.join(ROOT, 'examples', 'universal-stress', 'k329-k332-compiler-simulation-scientific-ai-generation-contract.v0.1.json');
const DEFAULT_RUNTIME_CONTRACT_PATH = path.join(ROOT, 'examples', 'universal-stress', 'k329-k332-compiler-simulation-scientific-runtime-contract.v0.1.json');
const DEFAULT_RUNTIME_EVIDENCE_PATH = path.join(ROOT, 'examples', 'universal-stress', 'evidence', 'k329-k332-compiler-simulation-scientific-runtime-v0.1.json');
const DEFAULT_RECEIPT_DIR = path.join(ROOT, 'examples', 'universal-stress', 'evidence', 'k329-k332-compiler-simulation-scientific-ai-generate');
...
export function verifyK329K332RuntimeEvidence(options = {}) {
  ...
  if (evidence.reportRoot !== evidenceRoot({ ...evidence, generatedAt: undefined, reportRoot: undefined })) throw new Error('RCL_K329_K332_RUNTIME_REPORT_ROOT_MISMATCH');
  if (evidence.contractRoot !== evidenceRoot(contract)) throw new Error('RCL_K329_K332_RUNTIME_CONTRACT_ROOT_MISMATCH');
  ...
  if (evidence.status !== 'PASS'
    ...
    || evidence.performance?.combinedP95Ms > contract.performanceBudget.combinedP95MsMax) throw new Error('RCL_K329_K332_RUNTIME_EVIDENCE_INVALID');
...
export function verifyK329K332GithubAuthorityBinding(options = {}) {
  ...
  if (authority.authorityRoot !== evidenceRoot({ ...authority, authorityRoot: undefined })) throw new Error('RCL_K329_K332_GITHUB_AUTHORITY_ROOT_MISMATCH');
  const jobs = authority.jobs ?? {};
  if (authority.format !== 'rcl.k329-k332.compiler-simulation-scientific-github-replay-authority.v0.1'
    ...
    || jobs.focused?.step?.name !== 'K329/K332 independent Compiler Simulation Scientific AI receipt replay'
    || jobs.focused?.step?.conclusion !== 'success'
    || jobs.windows?.name !== 'k01-windows-verification'
    || jobs.windows?.conclusion !== 'success'
    || jobs.windows?.step?.name !== 'K329/K332 Windows native Compiler Simulation Scientific runtime replay'
    || jobs.windows?.step?.conclusion !== 'success') throw new Error('RCL_K329_K332_GITHUB_AUTHORITY_INVALID');
  if (!/^[0-9a-f]{40}$/u.test(authority.sourceCommit)
    ...
    || !Number.isSafeInteger(jobs.windows?.id)) throw new Error('RCL_K329_K332_GITHUB_AUTHORITY_IDENTITY_INVALID');
  if (authority.contractRoot !== evidenceRoot(contract)
    || authority.localReceiptReportRoot !== localReport.reportRoot
    || authority.runtimeEvidenceBindingRoot !== evidenceRoot(localReport.runtimeEvidenceBinding)) throw new Error('RCL_K329_K332_GITHUB_AUTHORITY_EVIDENCE_MISMATCH');
  return {
    admitted: true,
    status: 'PASS_GITHUB_LINUX_WINDOWS_NATIVE_SIMULATION_SCIENTIFIC_REPLAY_BOUND',
...
export function verifyK329K332CompilerSimulationScientificReceipt(options = {}) {
  const contractPath = path.resolve(options.contractPath ?? DEFAULT_CONTRACT_PATH);
  const receiptDir = path.resolve(options.receiptDir ?? DEFAULT_RECEIPT_DIR);
  ...
  if (!fs.existsSync(receiptPath)) return {
    ...
    verdict: 'K329_K332_AI_GENERATION_RECEIPT_MISSING',
    githubAuthority: verifyK329K332GithubAuthorityBinding({ ...options, contractPath, receiptDir }),
  };
  const contract = readJson(contractPath);
  const report = readJson(receiptPath);
  const canonicalPath = path.join(ROOT, contract.canonical.sourcePath);
  const canonical = fs.readFileSync(canonicalPath, 'utf8');
  if (sha256(canonical) !== contract.canonical.sourceSha256) throw new Error('RCL_K329_K332_CANONICAL_SOURCE_DRIFT');
  const canonicalVerification = verifyK329K332CompilerSimulationScientificCandidate({ sourcePath: canonicalPath });
  if (canonicalVerification.status !== 'PASS' || canonicalVerification.reportRoot !== contract.canonical.candidateReportRoot) throw new Error('RCL_K329_K332_CANONICAL_VERIFICATION_MISMATCH');
  if (report.contractRoot !== evidenceRoot(contract)) throw new Error('RCL_K329_K332_AI_CONTRACT_ROOT_MISMATCH');
  if (report.reportRoot !== evidenceRoot({ ...report, generatedAt: undefined, reportRoot: undefined })) throw new Error('RCL_K329_K332_AI_REPORT_ROOT_MISMATCH');
  if (report.requiredTrials !== contract.requiredTrials || report.trials?.length !== contract.requiredTrials) throw new Error('RCL_K329_K332_AI_TRIAL_COUNT_MISMATCH');
  const results = [];
  for (const trialContract of contract.trials) {
    const receipt = report.trials.find((trial) => trial.trialId === trialContract.id);
    if (!receipt) throw new Error(`RCL_K329_K332_AI_TRIAL_MISSING:${trialContract.id}`);
    if (receipt.receiptRoot !== evidenceRoot({ ...receipt, receiptRoot: undefined })) throw new Error(`RCL_K329_K332_AI_TRIAL_ROOT_MISMATCH:${trialContract.id}`);
    const mutation = K329_K332_AI_GENERATION_MUTATIONS[trialContract.id];
    const mutated = replaceExactlyOnce(canonical, mutation.old, mutation.replacement, `RCL_K329_K332_MUTATION_SITE_INVALID:${trialContract.id}`);
    if (receipt.mutatedSourceSha256 !== sha256(mutated)) throw new Error(`RCL_K329_K332_MUTATED_HASH_MISMATCH:${trialContract.id}`);
    const candidate = replaceExactlyOnce(mutated, receipt.proposal.old, receipt.proposal.new, `RCL_K329_K332_SAVED_EDIT_NOT_EXACT:${trialContract.id}`);
    ...
    if (independence.oracleEditVisibleToGenerator !== false
      ...
      || receipt.generator.threadId.length === 0) throw new Error(`RCL_K329_K332_INDEPENDENCE_INVALID:${trialContract.id}`);
    ...
    try {
      ...
      verification = verifyK329K332CompilerSimulationScientificCandidate({ sourcePath: candidatePath });
    } finally {
  ...
  const runtime = verifyK329K332RuntimeEvidence(options);
  ...
  const githubAuthority = verifyK329K332GithubAuthorityBinding({ ...options, contractPath, receiptDir });
  ...
  return {
    ...
    verdict: admitted ? 'PASS_RECEIPT_REPLAY_GITHUB_LINUX_WINDOWS_NATIVE_SIMULATION_SCIENTIFIC_AUTHORITY_BOUND' : 'PASS_LOCAL_RECEIPT_GITHUB_AUTHORITY_REQUIRED',
  };
}
