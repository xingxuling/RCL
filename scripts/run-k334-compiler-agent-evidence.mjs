...
const DEFAULT_CONTRACT_PATH = path.join(ROOT, 'examples', 'universal-stress', 'k334-compiler-agent-runtime-contract.v0.1.json');
const DEFAULT_OUTPUT_PATH = path.join(ROOT, 'examples', 'universal-stress', 'evidence', 'k334-compiler-agent-runtime-v0.1.json');
...
function project(state) {
  return {
    success: state?.['result.success'], observeOnly: state?.['result.observe_only'],
    capabilityDenied: state?.['result.capability_denied'], budgetDenied: state?.['result.budget_denied'],
    unapproved: state?.['result.unapproved'], riskDenied: state?.['result.risk_denied'],
    killSwitch: state?.['result.kill_switch'],
  };
}
function independentOracle() {
  return {
    success: [1, 20, 11, 3, [10, 20], 1],
    observeOnly: [2, 20, 11, 0, [10, 20], 0],
    capabilityDenied: [-1, 0, 0, 0, [-30], 0],
    budgetDenied: [-2, 0, 0, 0, [-40], 0],
    unapproved: [-3, 0, 0, 0, [-20], 0],
    riskDenied: [-5, 0, 0, 0, [-50], 0],
    killSwitch: [-4, 0, 0, 0, [-4], 0],
  };
}

export function runK334CompilerAgentEvidence(options = {}) {
  ...
  if (contract.format !== 'rcl.k334.compiler-agent-runtime-contract.v0.1'
    || contract.frozenBeforeAcquisition !== true) throw new Error('RCL_K334_RUNTIME_CONTRACT_INVALID');
  ...
  for (const requiredPath of [sourcePath, compilerRbcPath, rclcPath, rclvmPath]) {
    if (!fs.existsSync(requiredPath)) throw new Error(`RCL_K334_REQUIRED_ARTIFACT_MISSING:${requiredPath}`);
  }
  const source = fs.readFileSync(sourcePath, 'utf8');
  if (sha256(source) !== contract.canonical.sourceSha256) throw new Error('RCL_K334_CANONICAL_SOURCE_DRIFT');
  if (sha256(fs.readFileSync(compilerRbcPath)) !== contract.canonical.compilerRbcSha256) throw new Error('RCL_K334_COMPILER_RBC_DRIFT');
  if (JSON.stringify(contract.expectedProjection) !== JSON.stringify(independentOracle())) throw new Error('RCL_K334_CONTRACT_ORACLE_DRIFT');

  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'rcl-k334-runtime-'));
  ...
  try {
    ...
    if (sha256(bootstrapBytecode) !== contract.canonical.bootstrapArtifactSha256) throw new Error('RCL_K334_BOOTSTRAP_ARTIFACT_DRIFT');
    for (let index = 0; index < contract.required.rounds; index += 1) {
      ...
      if (compilation.error || compilation.status !== 0) throw new Error(`RCL_K334_NATIVE_COMPILE_FAILED:${index}:${compilation.stderr}`);
      const nativeBytecode = fs.readFileSync(rbcPath);
      if (contract.required.bootstrapByteParity && !nativeBytecode.equals(bootstrapBytecode)) throw new Error(`RCL_K334_BOOTSTRAP_PARITY_FAILED:${index}`);
      const execution = run(rclvmPath, [rbcPath]);
      if (execution.error || execution.status !== 0) throw new Error(`RCL_K334_NATIVE_EXECUTE_FAILED:${index}:${execution.stderr}`);
      const payload = verifyNativeSemanticStateRoot(parseJson(execution.stdout, 'RCL_K334_NATIVE_JSON_INVALID'), {
        requireNativeRoot: contract.required.nativeSemanticStateRoot,
      });
      const observed = project(payload.state);
      if (JSON.stringify(observed) !== JSON.stringify(contract.expectedProjection)) throw new Error(`RCL_K334_EXPECTED_STATE_MISMATCH:${index}`);
      if (contract.required.independentJavascriptOracle && JSON.stringify(observed) !== JSON.stringify(independentOracle())) throw new Error(`RCL_K334_ORACLE_MISMATCH:${index}`);
    ...
    const controls = [
      ['capabilityMutationRejected', 'action_capability(action) != granted_capability', 'action_capability(action) == granted_capability'],
      ['budgetMutationRejected', 'action_cost(action) > budget', 'action_cost(action) < budget'],
      ['approvalMutationRejected', 'action_requires_approval(action) == 1 and human_approval == 0', 'action_requires_approval(action) == 1 and human_approval == 1'],
      ['selectionMutationRejected', 'action_score(sequence_get(actions, cursor)) > best_score', 'action_score(sequence_get(actions, cursor)) < best_score'],
    ];
    for (const [name, oldText, newText] of controls) {
      const mutatedSource = replaceExactlyOnce(source, oldText, newText, `RCL_K334_MUTATION_SITE_INVALID:${name}`);
      ...
      const payload = verifyNativeSemanticStateRoot(parseJson(execution.stdout, `RCL_K334_${name}_JSON_INVALID`), { requireNativeRoot: true });
  ...
  const withoutRoot = {
    format: 'rcl.k334.compiler-agent-runtime-evidence.v0.1', generatedAt: new Date().toISOString(),
    ...
    oracle: { implementation: 'independent-javascript-bounded-agent-state-machine', expected: independentOracle() },
...
if (isMain) {
  const result = runK334CompilerAgentEvidence();
