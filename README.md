...
It now ships a real Windows native VM, a bytecode format, staged self-hosting artifacts, fileized source emission, and a self-verifying compiler/runtime takeover ladder. The current verified ceiling is **Stage30: RCL-owned general expression parser subset**: an RCL artifact runs inside `native/rclvm.exe`, parses a two-rule RCL source program with general expression precedence, emits target RBC bytecode, and matches the JavaScript compiler reference byte-for-byte.
...
stage0-stage30: verified by the sequential self-host ladder
...
node scripts/verify-rcl-selfhost-stage30.mjs
...
RCL is not claiming the whole compiler/runtime has already escaped JavaScript. The current build proves a native-running self-host ladder through Stage30, not total closure. The next real fight is RCL-owned rule lowering loops and general bytecode emission, then removing JS from the authority path instead of merely using it as a reference.
