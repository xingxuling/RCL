...
It now ships a real Windows native VM, a bytecode format, staged self-hosting artifacts, fileized source emission, and a self-verifying compiler/runtime takeover ladder. The current verified ceiling is **Stage32: RCL-owned facet/warrant parser subset**: an RCL artifact runs inside `native/rclvm.exe`, parses facet, warrant, rule and directive sequences, emits initial state/warrant bytecode through RCL-owned loops, lowers six directive invocations, and matches the JavaScript compiler reference byte-for-byte.
...
base lowering: four facets, three warrants, eleven generated base instructions
rule lowering: three rules, six directive invocations, 120 target instructions
stage0-stage32: verified by the sequential self-host ladder
...
node scripts/verify-rcl-selfhost-stage32.mjs
...
RCL is not claiming the whole compiler/runtime has already escaped JavaScript. The current build proves a native-running self-host ladder through Stage32, not total closure. The next real fight is parser completion, broader compiler self-emission, and removing JS from the authority path instead of merely using it as a reference.
