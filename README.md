...
It now ships a real Windows native VM, a bytecode format, staged self-hosting artifacts, fileized source emission, and a self-verifying compiler/runtime takeover ladder. The current verified ceiling is **Stage31: RCL-owned rule lowering loop subset**: an RCL artifact runs inside `native/rclvm.exe`, parses a three-rule RCL source program, builds rule and directive sequences, lowers six directive invocations through an RCL-owned bytecode emission loop, and matches the JavaScript compiler reference byte-for-byte.
...
rule lowering: three rules, six directive invocations, 117 target instructions
stage0-stage31: verified by the sequential self-host ladder
...
node scripts/verify-rcl-selfhost-stage31.mjs
...
RCL is not claiming the whole compiler/runtime has already escaped JavaScript. The current build proves a native-running self-host ladder through Stage31, not total closure. The next real fight is parser completion, broader compiler self-emission, and removing JS from the authority path instead of merely using it as a reference.
