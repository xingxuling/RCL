...
It now ships a real Windows native VM, a bytecode format, staged self-hosting artifacts, fileized source emission, and a self-verifying compiler/runtime takeover ladder. The current verified ceiling is **Stage35: RCL-owned equality expression lowering subset**: an RCL artifact runs inside `native/rclvm.exe`, parses multiple `subject` blocks, preserves subject ownership on every warrant, lowers Text and Truth equality expressions into `EQ` bytecode, emits five subject-aware `GRANT_WARRANT` instructions, lowers five rules and ten directive invocations, and matches the JavaScript compiler reference byte-for-byte.
...
expression lowering: Text equality and Truth equality emit PUSH_STRING/PUSH_BOOL plus EQ
rule lowering: five rules, ten directive invocations, 196 target instructions
stage0-stage35: verified by the sequential self-host ladder
...
node scripts/verify-rcl-selfhost-stage35.mjs
