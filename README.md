...
It now ships a real Windows native VM, a bytecode format, staged self-hosting artifacts, fileized source emission, and a self-verifying compiler/runtime takeover ladder. The current verified ceiling is **Stage38: RCL-owned boolean connective lowering subset**: an RCL artifact runs inside `native/rclvm.exe`, parses multiple `subject` blocks, preserves subject ownership on every warrant, lowers `==`, `!=`, `<`, `<=`, `>` and `>=` into native comparison bytecode, maps `+`, `-`, `*` and `/` alter expressions into arithmetic bytecode, lowers two-clause `and` / `or` guard and preserve expressions into `AND` and `OR`, emits five subject-aware `GRANT_WARRANT` instructions, lowers five rules and ten directive invocations, and matches the JavaScript compiler reference byte-for-byte.
...
rule lowering: five rules, ten directive invocations, 276 target instructions
stage0-stage38: verified by the sequential self-host ladder
...
node scripts/verify-rcl-selfhost-stage38.mjs
