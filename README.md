...
It now ships a real Windows native VM, a bytecode format, staged self-hosting artifacts, fileized source emission, and a self-verifying compiler/runtime takeover ladder. The current verified ceiling is **Stage29: RCL-owned multirule expression source lowering subset**: an RCL artifact runs inside `native/rclvm.exe`, tokenizes and parses a two-rule RCL source program, emits target RBC bytecode, and matches the JavaScript compiler reference byte-for-byte.
...
node scripts/verify-rcl-selfhost-stage29.mjs
...
RCL is not claiming the whole compiler/runtime has already escaped JavaScript. The current build proves a native-running self-host ladder through Stage29, not total closure. The next real fight is RCL-owned rule lowering loops and general expression precedence, then removing JS from the authority path instead of merely using it as a reference.
