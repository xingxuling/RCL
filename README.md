...
It now ships a real Windows native VM, a bytecode format, staged self-hosting artifacts, fileized source emission, and a self-verifying compiler/runtime takeover ladder. The current verified ceiling is **Stage34: RCL-owned multisubject warrant parser subset**: an RCL artifact runs inside `native/rclvm.exe`, parses multiple `subject` blocks, preserves subject ownership on every warrant, emits five subject-aware `GRANT_WARRANT` instructions, lowers five rules and ten directive invocations, and matches the JavaScript compiler reference byte-for-byte.
...
base lowering: four facets, five warrants across two subjects, thirteen generated base instructions
rule lowering: five rules, ten directive invocations, 194 target instructions
stage0-stage34: verified by the sequential self-host ladder
...
node scripts/verify-rcl-selfhost-stage34.mjs
...
RCL is not claiming the whole compiler/runtime has already escaped JavaScript. The current build proves a native-running self-host ladder through Stage34, not total closure. The next real fight is parser completion, broader compiler self-emission, and removing JS from the authority path instead of merely using it as a reference.
