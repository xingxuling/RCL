...
It now ships a real Windows native VM, a bytecode format, staged self-hosting artifacts, fileized source emission, and a self-verifying compiler/runtime takeover ladder. The current verified ceiling is **Stage36: RCL-owned primitive comparison operator lowering subset**: an RCL artifact runs inside `native/rclvm.exe`, parses multiple `subject` blocks, preserves subject ownership on every warrant, lowers `==`, `!=`, `<`, `<=`, `>` and `>=` into native comparison bytecode, emits five subject-aware `GRANT_WARRANT` instructions, lowers five rules and ten directive invocations, and matches the JavaScript compiler reference byte-for-byte.
...
stage0-stage36: verified by the sequential self-host ladder
...
node scripts/verify-rcl-selfhost-stage36.mjs
