...
RCL is no longer a polite little DSL pretending to be architecture notes.

It now ships a real Windows native VM, a bytecode format, staged self-hosting artifacts, fileized source emission, and a self-verifying compiler/runtime takeover ladder. The current verified ceiling is **Stage39: RCL-owned unary not lowering subset**: an RCL artifact runs inside `native/rclvm.exe`, parses multiple `subject` blocks, preserves subject ownership on every warrant, lowers `==`, `!=`, `<`, `<=`, `>` and `>=` into native comparison bytecode, maps `+`, `-`, `*` and `/` alter expressions into arithmetic bytecode, lowers two-clause `and` / `or` guard and preserve expressions into `AND` and `OR`, lowers parenthesized primitive comparisons wrapped in unary `not` into comparison bytecode followed by `NOT`, emits five subject-aware `GRANT_WARRANT` instructions, lowers five rules and ten directive invocations, and matches the JavaScript compiler reference byte-for-byte.

That is the line in the sand: RCL is becoming a language that can compile pieces of itself, execute them natively, and show receipts.

## Current Verified Ceiling

```text
stage39_rcl_owned_unary_not_lowering_subset
native/rclvm.exe: real Windows PE executable
target RBC: generated by RCL inside native VM and byte-identical to JS reference
base lowering: five facets, including Text literal state, plus five warrants across two subjects
expression lowering: primitive comparison operators emit EQ/NEQ/LT/LTE/GT/GTE
arithmetic lowering: alter expressions emit ADD/SUB/MUL/DIV from RCL-owned operator mapping
boolean lowering: two-clause guard/preserve expressions emit AND/OR from RCL-owned connective mapping
unary lowering: not (...) comparison expressions emit NOT from RCL-owned unary mapping
parser subset: single-symbol < and > comparisons are parsed by the RCL artifact, not waved through by README prose
rule lowering: five rules, ten directive invocations, 316 target instructions
stage0-stage39: verified by the sequential self-host ladder
fileize/restore: verified source package round trip
full self-hosting: not yet
JS compiler/runtime: still used as reference and harness
```
...
node scripts/verify-rcl-selfhost-stage39.mjs
node scripts/verify-rcl-selfhost-all.mjs
node scripts/fileize-rcl-source.mjs
node scripts/restore-rcl-fileized-source.mjs
```

## Honest Boundary

RCL is not claiming the whole compiler/runtime has already escaped JavaScript. The current build proves a native-running self-host ladder through Stage39, not total closure. The next real fight is expression AST completion, parser completion, broader compiler self-emission, and removing JS from the authority path instead of merely using it as a reference.

Still: this is not a prompt costume. This is a compiler/runtime stack learning to own its own bloodstream.
