...
import {
  emitNativeUiAndroidActivity,
  lowerNativeUiToAndroid,
  simulateNativeUiAndroidApplication,
  traceNativeUiAndroidApplication,
} from './ui/android-ui-backend.mjs';
import { RCL_NATIVE_UI_ANDROID_FORMAT } from './ui/ui-schema.mjs';
...
export function compileRclAndroidApplication(rclSource, androidSpec) {
  ...
  if ((program.nativeUis ?? []).length > 0) {
    ...
    const target = androidSpec ?? {};
    if (target.schema && target.schema !== 'rcl.native-ui.android-target.v0.1') throw new Error(`RCL_UI_ANDROID_TARGET_SCHEMA:${target.schema}`);
    return lowerNativeUiToAndroid(program.nativeUis[0], target);
  }
...
export function emitNativeAndroidActivity(manifest) {
  if (manifest.schema === RCL_NATIVE_UI_ANDROID_FORMAT) return emitNativeUiAndroidActivity(manifest);
...
export function buildRclAndroidApplication({ rclPath, specPath, outputPath }) {
  ...
  return Object.freeze({
    ...
    coverageMode: manifest.schema === RCL_NATIVE_UI_ANDROID_FORMAT ? 'native-semantic-candidate' : 'lowered-execution',
  });
}
...
export function simulateRclAndroidApplication(manifest, events = []) {
  if (manifest.schema === RCL_NATIVE_UI_ANDROID_FORMAT) return simulateNativeUiAndroidApplication(manifest, events);
...
export { traceNativeUiAndroidApplication };
