...
function emitCss(ui) {
  const base = 'html,body{margin:0;min-height:100%;box-sizing:border-box}*,*:before,*:after{box-sizing:inherit}body{display:grid;place-items:center;padding:24px;font-family:system-ui,sans-serif}[data-rcl-role="action"]{border:0;padding:10px 16px;cursor:pointer}[data-rcl-role="input"]{padding:10px 12px}';
...
function runtimeScript(manifest) {
  ...
  return `(()=>{const manifest=${payload};const ui=manifest.ui;const state=Object.fromEntries(ui.state.map(x=>[x.id,structuredClone(x.initial)]));const trace=[];const lifecycle=[];const nodes=new Map();(function walk(n){nodes.set(n.id,n);for(const c of n.children)walk(c)})(ui.viewTree);
...
lifecycleStage('create');render();lifecycleStage('activate');lifecycleStage('resume');document.addEventListener('visibilitychange',()=>lifecycleStage(document.visibilityState==='hidden'?'suspend':'resume'));window.addEventListener('pageshow',()=>lifecycleStage('activate'));window.addEventListener('pagehide',()=>{lifecycleStage('suspend');lifecycleStage('destroy')});window.RCLNativeUI={manifest,state,trace,lifecycle,dispatch,render,rendered,lifecycleStage};
