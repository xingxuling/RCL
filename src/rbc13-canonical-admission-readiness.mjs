...
export function buildRbc13CanonicalAdmissionReadiness(input = {}) {
  ...
  const aiPass = aiGenerate.status === 'CANDIDATE'
    && aiGenerate.gate === 'PASS'
    && Number(aiGenerate.successfulTrials ?? 0) >= Number(aiGenerate.requiredTrials ?? 1);
  const universalPass = universal.status === 'PASS' && universal.universalGrowthEligible === true;
  const gates = {
    ...
    A3_legacyRegressionClosure: gate('A3_legacyRegressionClosure', legacy.v1FocusedStatus === 'VERIFIED' && legacy.fullSuiteStatus === 'VERIFIED', [legacy.v1FocusedRoot, legacy.fullSuiteRoot], legacy.fullSuiteStatus === 'VERIFIED' ? null : 'Legacy/full-suite regression closure is not VERIFIED'),
    ...
    A10_aiGenerateDonor: gate('A10_aiGenerateDonor', aiPass, [aiGenerate.root, aiGenerate.responseRoot, aiGenerate.corpus?.root], aiPass ? null : `AI_GENERATE donor status is ${aiGenerate.status ?? 'missing'}`),
    ...
    A12_universalStressAdmissionCell: gate('A12_universalStressAdmissionCell', universalPass, [universal.root], universal.status === 'PASS' ? null : `Universal Stress cell is ${universal.status ?? 'missing'}`),
  };
