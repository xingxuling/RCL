...
import { assessRclApplicationFrameworkCatalog, buildRclApplicationFramework, listRclApplicationFrameworks, verifyRclApplicationFrameworkBuild } from './index.mjs';
import { assessRclNativeFrameworkAtlas, listRclNativeFrameworks } from './index.mjs';
...
if (command === 'framework-atlas') {
  console.log(JSON.stringify({
    assessment: assessRclNativeFrameworkAtlas(),
    frameworks: listRclNativeFrameworks(),
  }, null, 2));
  process.exit(0);
}
if (command === 'application-frameworks') {
  console.log(JSON.stringify({
    assessment: assessRclApplicationFrameworkCatalog(),
    frameworks: listRclApplicationFrameworks(),
  }, null, 2));
  process.exit(0);
}
if (command === 'application-framework-build') {
  if (!file || !output) {
    console.error('Usage: rcl application-framework-build <file.rcl> <output-dir> [framework-spec.json]');
    process.exit(1);
  }
  console.log(JSON.stringify(buildRclApplicationFramework({ rclPath: file, outputPath: output, specPath: third }), null, 2));
  process.exit(0);
}
if (command === 'application-framework-verify') {
  if (!file) {
    console.error('Usage: rcl application-framework-verify <output-dir>');
    process.exit(1);
  }
  console.log(JSON.stringify(verifyRclApplicationFrameworkBuild(file), null, 2));
  process.exit(0);
}
