...
The K09-K16 accelerator sequence now contains bounded OpenCL candidates for persistent execution, batched execution, gradient pairs, cross-node execution, buffer arenas, Tensor-input residency, ordered intermediate graph residency and GPU-native additive masked softmax. K15 remains the residency frontier: on the local AMD gfx1152 device it executed a two-node BF16 graph with output `4040`, zero intermediate readbacks and one final readback, five allocations / `22` bytes / five releases, three host-to-device uploads and one device-to-host readback. K16 adds a direct generic Tensor masked-softmax lowerer: the same host executed a rank-2 `2 x 3` additive-mask fixture with exact BF16 output bits `3f80 31c1 3283 323f 3f3b 3e8a`, exact independent CPU differential and deterministic root `959537aaf0115e819ad927a3d1fc3ec6eff6a9dbba086c964460f5036f4c9e03`; its local suite is `3/3` passing. Hosted PR #125 and post-merge `main@c13a573` passed K16, K09-K15, Authority, Canonical and Universal scopes on Ubuntu/Windows where applicable.
...
K15 grants only an ordered ephemeral graph-residency candidate and an intermediate device-resource candidate; K16 grants only an AMD OpenCL BF16 additive masked-softmax lowering candidate and a GPU-native additive-mask candidate. Neither grants full-graph or training-step residency, GPU-native Autodiff/AdamW, GPU training, parallel execution, throughput, VRAM, portability, RCL-10M/RCL-1B or production-model claims. The K400 matrix remains `23 PASS / 0 BLOCKED / 377 UNTESTED`, verdict `INCOMPLETE`; no K400 cell is promoted by K15 or K16.

## 14. Next absorption gate

The next highest-leverage technical gap is stricter full graph / training-step residency and integration of the GPU-native non-matmul path into a complete generic graph. The implementation must preserve generic RCL Tensor and attention semantics, extend positive/negative/boundary and CPU differential evidence across graph residency and training-step state, and keep unavailable hardware explicitly `NOT_RUN` or `BLOCKED`. A local accelerator result is not GPU-training proof, and compilation or hosted replay cannot substitute for final weights, sustained training, autoregressive generation or production corpus evidence.
