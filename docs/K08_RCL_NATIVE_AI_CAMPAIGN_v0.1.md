...
K15 grants only an ordered ephemeral graph-residency candidate and an intermediate device-resource candidate; K16 grants only an AMD OpenCL BF16 additive masked-softmax lowering candidate and a GPU-native additive-mask candidate; K17 grants only the bounded ordered mixed-graph candidate. None grants full-graph or training-step residency, GPU-native Autodiff/AdamW, GPU training, parallel execution, throughput, VRAM, portability, RCL-10M/RCL-1B or production-model claims. The K400 matrix remains `23 PASS / 0 BLOCKED / 377 UNTESTED`, verdict `INCOMPLETE`; no K400 cell is promoted by K15, K16 or K17.

## 14. Next absorption gate

The next highest-leverage technical gap is stricter full graph / training-step residency and integration of the remaining GPU-native non-matmul path into a complete generic graph. K17 closes only the two-node matmul-to-masked-softmax transport shape; the next candidate must preserve generic RCL Tensor and attention semantics, extend positive/negative/boundary and CPU differential evidence across graph residency and training-step state, and keep unavailable hardware explicitly `NOT_RUN` or `BLOCKED`. A local accelerator result is not GPU-training proof, and compilation or hosted replay cannot substitute for final weights, sustained training, autoregressive generation or production corpus evidence.
