...
K15 grants only an ordered ephemeral graph-residency candidate and an intermediate device-resource candidate; K16 grants only an AMD OpenCL BF16 additive masked-softmax lowering candidate and a GPU-native additive-mask candidate; K17 grants only the bounded ordered mixed-graph candidate; K18 grants only bounded full-graph residency, training-step resource reuse and the generic `add` path. None grants GPU-native Autodiff/AdamW, GPU training, full-graph training semantics, parallel execution, throughput, VRAM, portability, RCL-10M/RCL-1B or production-model claims. The K400 matrix remains `23 PASS / 0 BLOCKED / 377 UNTESTED`, verdict `INCOMPLETE`; no K400 cell is promoted by K15, K16, K17 or K18.

## 14. Next absorption gate

The next highest-leverage technical gap is GPU-native reverse/optimizer integration and the remaining production graph lifecycle. K18 closes only bounded repeated forward resource reuse for a three-node generic graph; the next candidate must preserve generic RCL Tensor and attention semantics while extending positive/negative/boundary and CPU differential evidence through GPU-native backward/optimizer state, then through real-corpus Transformer training and serving. A local accelerator result is not GPU-training proof, and compilation or hosted replay cannot substitute for final weights, sustained training, autoregressive generation or production corpus evidence.
