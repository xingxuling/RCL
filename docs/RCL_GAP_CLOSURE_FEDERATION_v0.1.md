...
- AI002: RCL shape admission and graph governance now bind an ordered reverse plan to the generic Rust CPU-f64 Autodiff provider; shape/edge/gradient-shape parity and fail-closed provider-profile checks are locally evidenced, while broader graph corpus, GPU-native reverse execution, canonical promotion and production scale remain open;
