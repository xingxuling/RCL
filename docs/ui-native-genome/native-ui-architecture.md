...
The canonical self-host compiler now owns the minimal UI, exact Counter state/derived/lifecycle/theme/style/tree/binding/layout/local-event semantic slice, typed or standard-inferred UI-local parameters, governed `reality-transaction` declarations, fixed width/height intent and canonical in-app navigation with byte-identical fixed-point, differential, mutation, normalization and negative-control evidence. Whole-program validation binds every governed UI rule reference to a declared emergence rule, validates route identities and root-child targets, and rejects mixed local/reality handlers or multiple transitions. Fixed values are non-negative numeric literals, normalize equivalent numeric spellings, and lower from one UI root to Web pixels and Android layout parameters; routes lower from the same root to Web and native Android visibility. This is compile-time semantic ownership only: actual reality execution remains in the external Gateway and yields a `CandidateReality`, never a UI-owned commit. Formal repository-wide promotion still requires resource/device-adaptation and fuller accessibility semantics, Android device evidence, version-contract changes and a human-reviewed promotion receipt.
