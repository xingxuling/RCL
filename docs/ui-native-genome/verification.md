...
- Canonical self-host parity is verified for the minimal UI, exact Counter slice, and typed or standard-inferred UI-local event parameters. Invalid parameter signatures and reality-transaction UI events fail closed; fixed sizing and wider candidate forms remain separately unabsorbed.
