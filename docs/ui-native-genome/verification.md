...
The SHA-bound Integration Court report for `native-ui-genome-v0.1-candidate-8-selfhost-device-adaptation` records 5 `BLOCKED`, 395 `UNTESTED`, 0 `PASS` and maturity `U0`, with deterministic report root `b23310ad9a03dacf876e972670d9d02ca2cc49281d74fd9d1852b4c8055d58fa`. Two successive report runs returned that same root after the volatile `generatedAt` field was removed from the root preimage; the focused Universal Stress suite passed 18/18. Device-adaptation and compiler-performance evidence do not compensate for the independent browser-performance, Android-device or `AI_GENERATE` gaps.
