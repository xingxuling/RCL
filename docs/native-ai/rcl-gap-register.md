...
`byte/general tokenizer -> positional genome (RoPE candidate) -> multi-head attention -> multi-block Tiny LM -> GPU/mixed precision -> DistributedTensor -> RCL-10M -> RCL-100M -> RCL-300M -> RCL-1B`.
