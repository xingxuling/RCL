...
| `RCL_GAP_AI_016` | Accelerator and mixed-precision AI execution | Current admitted model evidence executes CPU f64 | Backend / Performance Gap | cross-model | Lower canonical Tensor/Autodiff semantics to GPU/NPU and f32/bf16/fp16 organs with bounded numerical parity; this is now the highest-leverage model-scale blocker | Large-model feasibility |
