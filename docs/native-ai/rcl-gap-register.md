...
| `RCL_GAP_AI_003` | Tensor-connected execution of RCL-owned Momentum/Adam/AdamW semantics | K08-H hosted Optimizer Genome proves model-agnostic scalar SGD/Momentum/Adam/AdamW semantics and checkpoint parity; current Autodiff training organ still updates Tensor parameters with Batch SGD | Backend / Lowering Gap | cross-model | Canonical optimizer semantics absorbed as ENGINE-E3 candidate; next lower generic Tensor parameter/gradient/state tuples through the same contract | Tiny LM and future large-model training |
...
| `RCL_GAP_AI_012` | Self-hosted lowering from RCL Transformer topology to the complete Tensor/Autodiff graph | K08-I RCL semantic genome owns decoder-block topology/invariants while the hosted test organ constructs the generic Tensor SSA graph | Compiler / Lowering Gap | Transformer-family and future graph models | ENGINE-E4 first decoder block is hosted-evidence bound; absorb graph construction into RCL-owned model/lowering infrastructure before canonical promotion | Tiny LM, Transformer scale and AI_GENERATE ownership |
...
`Tensor AdamW bridge or bounded SGD training -> token/embedding + cross-entropy -> Tiny Decoder LM -> multi-head composition -> accelerator -> DistributedTensor`.
