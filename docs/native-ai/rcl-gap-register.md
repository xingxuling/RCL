...
| `RCL_GAP_AI_014` | Production-scale positional semantics integration | K08-N now owns pairwise RoPE semantics, rooted position-frame identity and generic `matmul/mul/add` lowering; the native f64 organ only materializes trig tables | Canonical Semantic / Model Integration Gap | sequence/attention models | **Bounded RoPE semantics absorbed as hosted candidate.** Remaining work is per-head application inside multi-head/GQA, long-context scale evidence, BF16/GPU numerical envelopes and final RCL-1B configuration binding | Context-sensitive LM quality and long-context cells |
| `RCL_GAP_AI_015` | Multi-head and multi-block Transformer composition | K08-I/J prove a one-head, one-block causal decoder path; K08-N supplies reusable RoPE Q/K position semantics | Model / Lowering Gap | Transformer-family | Generalize head partition/merge, GQA KV sharing and repeated block construction without model-special opcodes | Modern LLM architecture coverage |
...
`multi-head/GQA with RoPE -> multi-block parametric LM -> production multilingual+code corpus + actual ~64K vocabulary artifact -> GPU/mixed precision -> DistributedTensor -> RCL-10M -> RCL-100M -> RCL-300M -> RCL-1B`.