...
| `RCL_GAP_AI_016` | Accelerator and mixed-precision AI execution | Bounded AMD OpenCL BF16 matmul lowerer is now bound into an explicit RCL BF16 Autodiff + AdamW hybrid graph; non-matmul nodes remain explicitly RCL CPU reference | Backend / Performance Gap | cross-model | Partially reduced by an OpenCL hybrid candidate with real local AMD matmul execution, CPU loss/parameter/state/checkpoint differential and fail-closed placement/provider boundaries. Full-graph GPU, GPU backward/optimizer kernels, multi-block GQA/RoPE GPU, throughput and generic GPU portability remain open | Large-model feasibility |
...
`GPU/mixed precision -> multi-block AdamW replay + production multilingual/code corpus + actual ~64K vocabulary artifact -> RCL-10M -> DistributedTensor -> RCL-100M -> RCL-300M -> RCL-1B`.
