...
| `RCL_GAP_AI_002` | General computation graph and reverse-mode Autodiff | K08-G RCL Autodiff Genome plus Rust reverse-mode execution organ; JS remains analytic/finite-difference oracle only | Canonical Semantic Gap | cross-model | Locally absorbed as ENGINE-E2 candidate for the recorded primitive set; GitHub replay, optimizer integration, broader graph corpus and promotion remain open | K233 and future learnable-system cells |
...
`GitHub replay of native Autodiff -> Optimizer Genome (SGD/Momentum/Adam/AdamW) with rooted optimizer-state checkpoint parity -> Transformer primitives -> first Decoder block -> Tiny LM`.
